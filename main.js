...
$(function() {
  ...
  function set_settings() {
    ...
    settings.defaultredirect = 'https://baselpsychology.eu.qualtrics.com/jfe/form/SV_a03gHW2F11TSINE';
  ...
  function init_name() {
  	...
  	$('#submit_username').on('click',function() {
  		...
  		if(uname == "") {
  			error = 1;
  			errormsg = 'Bitte füge hier einen Text ein';
  			uname = "undefined";
  		}
    	...
    	$('#submit_avatar').on('click',function() {
    		if($('.selected').length == 1) {
  			...
  			window.avatarexport = /avatar_([^\s]+)/.exec(window.avatar)[1];
    			...
    			alertify.log("Bitte wähle einen Avatar","error");
    		}
    	});
  ...
  function init_text() {
  	...
  	$("#description").keyup(function(){
  	  $("#count").text("Verbleibende Zeichen: " + (600 - $(this).val().length));
  	});
  	...
  	$('#submit_text').on('click',function() {
  		...
  		if($('#description').val() == "") {
  			error = 1;
  			errormsg = 'Bitte schreibe einen Text';
  		}
    ...
    // When user receives likes
	  $('.userslikes').each(function() {
  		...
  		{ 
  			...
  			themsg = usernames[i] + " hat deinen Post geliked";
  ...
  $(window).bind('beforeunload', function(){
    return 'Bist du sicher, dass du die Studie abbrechen möchtest?';
  });   
